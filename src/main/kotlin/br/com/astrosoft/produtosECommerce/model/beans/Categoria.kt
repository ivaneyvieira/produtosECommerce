...
data class Categoria(var categoriaNo: Int = 0,
  ...
  val codigo
    get() = categoriaNo.toString()
      .lpad(6, "0")
  val codigoGrupo
    get() = codigo.substring(0, 2)
              .toIntOrNull() ?: 0
  val codigoDepartamento
    get() = codigo.substring(2, 4)
              .toIntOrNull() ?: 0
  val codigoSecao
    get() = codigo.substring(4, 6)
              .toIntOrNull() ?: 0
  val descricao
    get() = when {
      grupo == ""        -> ""
  ...
  companion object {
    fun makeCodigo(grupo: Int, departamento: Int, secao: Int) = "${grupo.toString()
      .lpad(2, "0")}${departamento.toString()
      .lpad(2, "0")}${secao.toString()
      .lpad(2, "0")}"
    ...
    fun listGrupo() = local.findAllCategoria()
      .distinctBy {it.grupo}
      .sortedBy {it.categoriaNo}
    
    fun listDepartamento(grupo: String) = local.findAllCategoria()
      .filter {it.grupo == grupo}
      .distinctBy {it.departamento}
      .sortedBy {it.categoriaNo}
    
    fun listSecao(grupo: String, departamento: String) = local.findAllCategoria()
