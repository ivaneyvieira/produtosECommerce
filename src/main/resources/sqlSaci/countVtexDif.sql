...
WHERE (skuId LIKE CONCAT(@SKU, '%') OR @SKU = '')
  ...
  AND (ROUND(refprice*100) = @PRECO  OR ROUND(preco*100) = @PRECO
  OR ROUND(precoList*100) = @PRECO  OR ROUND(promoprice*100) = @PRECO OR @PRECO = 0)
  ...
  AND ((IFNULL(promoprice, 0) != IFNULL(promoVtex, 0) AND :diferenca = 'PROMO') OR
       ...
       (IFNULL(preco, 0) != IFNULL(promoprice, 0) AND IFNULL(promoprice, 0) != 0
